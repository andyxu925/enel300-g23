...
int main(void)
{
    ...
    while (1)
    {
        ...
        uint8_t last_state = HAL_GPIO_ReadPin(GPIOC, GPIO_PIN_2);

        // Count pulses for 200 ms
        ...
        // Threshold (you will tune this Monday)
        if (pulse_count < 0.2 * 13000)
            HAL_GPIO_WritePin(GPIOB, GPIO_PIN_14, GPIO_PIN_SET); // Metal detected
        else
            HAL_GPIO_WritePin(GPIOB, GPIO_PIN_14, GPIO_PIN_RESET); // No metal
